...
if(os.path.exists(tools_library_path)):
    ...
    startup_script = os.path.join(tools_library_path, "programs\\max\\libs\\initialize\\tools_library.ms")
    if(not os.path.isdir(startup_folder)):
        os.mkdir(startup_folder)
    shutil.copyfile(startup_script, os.path.join(startup_folder, "tools_library.ms"))
